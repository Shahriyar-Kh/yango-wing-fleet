      ...
      <div className="container-x grid grid-cols-1 gap-10 py-14 md:grid-cols-4">
        ...
        <div>
          ...
          <ul className="mt-4 space-y-2 text-sm text-muted-foreground">
            ...
            <li><Link to="/registration" className="hover:text-gold">Register</Link></li>
          ...
          <ul className="mt-4 space-y-3 text-sm text-muted-foreground">
            {BRAND.phones.map((p) => (
              <li key={p} className="flex items-center gap-2">
                <Phone className="h-4 w-4 text-gold" /> <a href={`tel:${p.replace(/-/g, "")}`} className="hover:text-foreground">{p}</a>
              </li>
            ))}
            <li className="flex items-center gap-2">
              <MessageCircle className="h-4 w-4 text-gold" />
              <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="hover:text-foreground">WhatsApp Support</a>
            </li>
            <li className="flex items-center gap-2">
